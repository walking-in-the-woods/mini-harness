#!/usr/bin/env bash
# setup-experiments.sh — создание структуры docs/experiments/.
#
# Одноразовый скрипт. Создаёт:
#   docs/experiments/README.md
#   docs/experiments/micro-models-n100.md
#   docs/experiments/artifacts/            (копирует эталонные выходы)
#
# Артефакты берутся из workspace/output/. Если файлов нет — скрипт
# предупреждает и создаёт пустую папку artifacts/, чтобы её можно было
# заполнить вручную.
#
# Идемпотентен: существующие файлы не перезаписываются без --force.
#
# Использование:
#   bash scripts/setup-experiments.sh [--force]
set -euo pipefail
...
write_file() {
    ...
    if [[ -e "$path" && "$FORCE" -ne 1 ]]; then
        echo "[=] существует, пропускаю: $path"
        cat > /dev/null
        return
    fi
...
mkdir -p docs/experiments/artifacts
echo "[+] Дерево docs/experiments/ готово"

# ── docs/experiments/README.md ─────────────────────────────────────────────
write_file docs/experiments/README.md <<'EOF_README'
# Эксперименты
...
# ── docs/experiments/micro-models-n100.md ──────────────────────────────────
# Лог тестов 1-4. Пути в логах санитизированы: вместо /home/<user>/...
# стоит <project_root>, чтобы не публиковать имя пользователя.
write_file docs/experiments/micro-models-n100.md <<'EOF_MICRO'
...
copy_artifact() {
    ...
    if [[ -f "$src" ]]; then
        cp "$src" "$dst"
        echo "[+] скопирован артефакт: $dst"
    else
        echo "[!] не найден источник: $src"
        echo "    (артефакт нужно скопировать вручную после прогона теста)"
    fi
}

copy_artifact workspace/output/article_summary.md \
              docs/experiments/artifacts/article_summary.md
copy_artifact workspace/output/article_facts.md \
              docs/experiments/artifacts/article_facts.md
copy_artifact workspace/output/code/counter_extended.py \
              docs/experiments/artifacts/counter_extended.py
copy_artifact workspace/output/code/text_utils_documented.py \
              docs/experiments/artifacts/text_utils_documented.py

# ── Итог ───────────────────────────────────────────────────────────────────
echo
echo "[+] Готово."
echo
echo "Проверьте и закоммитьте:"
echo "    git status docs/experiments/"
echo "    git add docs/experiments/"
echo "    git commit -m 'docs(experiments): add micro-model tests for N100/8GB'"
